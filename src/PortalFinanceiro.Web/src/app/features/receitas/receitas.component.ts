...
import { ReceitaRecorrenteRepository } from '../../core/repositories/recorrente.repository';
...
import { ReceitaMensalRepository } from '../../core/repositories/lancamento-mensal.repository';
...
@Component({
  ...
  template: `
    <div class="page">
      <app-section-header
        title="Receitas"
        subtitle="Gerencie suas receitas recorrentes e mensais"
      ...
      @if (tabAtiva() === 'recorrentes') {
        @if (loading()) {
          ...
          <app-empty-state title="Nenhuma receita recorrente" description="Cadastre sua primeira receita recorrente." actionLabel="Nova receita" (action)="abrirModal()" />
        } @else {
      ...
      @if (tabAtiva() === 'mensal') {
        ...
        @if (loading()) {
          ...
          <app-empty-state title="Nenhum lançamento neste mês" description="Cadastre uma receita recorrente para gerar lançamentos." actionLabel="Ir para Recorrentes" (action)="tabAtiva.set('recorrentes')" />
        } @else {
          <div class="table-card">
            <table class="table">
...
export class ReceitasComponent implements OnInit {
  ...
  private repo = inject(ReceitaRecorrenteRepository);
  private repoMensal = inject(ReceitaMensalRepository);
  ...
  async salvar(data: RecorrenteRequest) {
    ...
    try {
      if (this.editando()) {
        ...
        this.notify.success('Receita criada');
      }
      this.fecharModal();
  ...
  async receber(lancamento: LancamentoMensal) {
    try {
      await firstValueFrom(this.repoMensal.receber(lancamento.id, { data: new Date().toISOString().split('T')[0] }));
  ...
  totalReceitas() { return this.mensais().reduce((s, l) => s + l.valor, 0); }
  totalRecebido() { return this.mensais().filter(l => l.status === 'Realizado').reduce((s, l) => s + l.valor, 0); }
  percRecebido() { const t = this.totalReceitas(); return t ? Math.round(this.totalRecebido() / t * 100) : 0; }
}
