...
internal static class ContaBancariaSql
{
    ...
    public static string ListarPorUsuario => $"SELECT {C} FROM {T} WHERE IdUsuario = @IdUsuario AND Ativo = 1 ORDER BY EhPadrao DESC, Nome";
    ...
    public static string LimparPadrao => $"UPDATE {T} SET EhPadrao = 0, DataAlteracao = GETDATE() WHERE IdUsuario = @IdUsuario AND EhPadrao = 1";
}
